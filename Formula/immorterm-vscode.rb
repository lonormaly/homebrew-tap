class ImmortermVscode < Formula
  desc "ImmorTerm VS Code integration - persistent terminal setup scripts"
  ...
  depends_on "lonormaly/tap/immorterm"

  def install
    # Install the project setup script
    bin.install "src/installer.sh" => "immorterm-setup"
  ...
  def caveats
    <<~EOS
      ImmorTerm VS Code integration has been installed!

      To set up persistent terminals for a project, run:
        immorterm-setup /path/to/your/project

      Or in your project directory:
        immorterm-setup .

      This will:
        1. Configure ImmorTerm for persistent sessions
        2. Set up VS Code terminal integration
        3. Install the terminal name sync extension

      After setup, your terminals will survive VS Code crashes!
  ...
  test do
    assert_match "ImmorTerm v", shell_output("#{bin}/immorterm-setup --version")
  end
end
